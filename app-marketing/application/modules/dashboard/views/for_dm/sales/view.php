				...
				<div class="m-portlet__body">
					<div class="m-section">
						...
						<div class="m-section__content">
							...
							<form action="<?= base_url('dashboard/all_sales/editProfile_action') ?>" method="POST" id="form" enctype="multipart/form-data">
								...
								$role_saya = $this->db->query('SELECT role_id FROM marketing WHERE id=' . sess('mkt'))->row_array()['role_id'];
								$rules = rules_edit_mitra();
								$rule  = $rules[$role_saya];

								if ($role_saya == '5' || $role_saya == '4')
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		...
		$("#file_foto, #file_ktp, #file_cv, #file_ijazah, #file_sertifikat_1, #file_sertifikat_2").change(function() {
			...
			$.ajax({
				url: "<?= base_url('dashboard/all_sales/upload_files') ?>",
				type: 'post',
				data: form_data,
				contentType: false,
				cache: false,
