...
class All_sales extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->helper(['mitra']);
		$this->load->model(['All_sales_model' => 'model']);
	}

	public function index()
	{
		$this->viewku->title("List Sales");
		$this->viewku->view("for_dm/sales/index");
	}
	...
	public function view($marketing_id)
	{
		...
		if ($query->num_rows() > 0) {
			...
			$this->viewku->title("List Sales");
			$this->viewku->view("for_dm/sales/view", $data);
		}
	}
		...
		if ($this->input->is_ajax_request()) {
			...
			if ($this->form_validation->run()) {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$json['success'] = true;
					$json['alert'] = 'Update profile berhasil. ' . ($alrt_email != '' ? $alrt_email : '');
				}
			} else {
	...
	public function excel()
	{
		...
		foreach ($this->model->get_all() as $data) {
			...
			if (in_array($data->id, $array_sales)) {
	...
	function fetch_team()
	{
		...
		foreach ($fetch_data as $r) {
			...
			if (in_array($r->id, $array_sales)) {
				...
				$sub_array[] =  '<div class="text-center"><a href="' . base_url('dashboard/all_sales/view/' . $r->id) . '" class="btn btn-sm btn-primary"><i class="fa fa-eye"></i></a></div>';
				$data[] = $sub_array;
				$no++;
			}
		}
