...
class Nasabah extends CI_Controller
{
	...
	{
		$nasabah = $this->db->where('nasabah_id', $id)->get('nasabah');
		...
		if ($nasabah->num_rows() > 0) {
			...
			$data = array_merge($nasabah->row_array(), [
				'button' => 'Update',
	...
	public function updateOn($id)
	{
		...
		if ($this->Nasabah_model->update($id, $data)) // call the method from the model
		{
			...
			$this->session->set_flashdata('message', 'akun di Aktifkan');
			redirect(site_url('adminarea/nasabahoff'));
		}
	}
	...
	public function update_action()
	{
		...
		$this->form_validation->set_rules('penyelesaian_perselisihan', 'penyelesaian perselisihan', 'trim|xss_clean|required');
		$this->form_validation->set_rules('wakil_pialang', 'wakil pialang', 'trim|xss_clean|required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($_POST['foto_npwp'] != NULL) {
				$data = array(
					...
					'wakil_pialang' => $this->input->post('wakil_pialang', TRUE),
			...
			if ($this->input->post('status', TRUE) == 'Approved') {
				...
				$dataReq = $this->Acc_request_model->get_last_by_id_nasabah($this->input->post('nasabah_id', TRUE));
				$request = array('status_request' => 'Checking');
				$this->Acc_request_model->update($dataReq->acc_request_id, $request);
			}
	...
	private function _sendEmail($email, $subject, $pesan)
	{
		...
		$send['email'] = $email;
		$send['subjek'] = $subject;
		$send['pesan'] = $this->load->view('template_email/email_data_checking', ['pesan' => 'Verifikasi data gagal: [' . $pesan . ']. Untuk melengkapi data anda, mohon lakukan login kemudian klik <a href="' . base_url('registercomplete') . '">link berikut</a>.'], true);;
		$msg = send_mailer($send);
	...
	public function review_perjanjian($id)
	{
		// $data['acc'] = $this->Acc_trading_model->get_by_id_nasabah($id);
		$data['nasabah'] = $row = $this->Nasabah_model->get_by_id($id);

		$data['title'] = 'Perjanjian Review';
		$data['subtitle'] = '';
		$data['crumb'] = [
			'Nasabah' => '',
		];
		// $data['code_js'] = 'nasabah/codejs';
		$data['page'] = 'nasabah/review_perjanjian';
		$this->load->view('template/backend', $data);
	}

	// perjanjian berdasrkan akun demo
	public function perjanjian_nasabah($id)
	{
		$data['acc'] = $this->Acc_demo_model->get_by_id_nasabah($id);
		$data['nasabah'] = $row = $this->Nasabah_model->get_by_id($id);
		...
		$data['page'] = 'nasabah/review_perjanjian';
		$this->load->view('template/backend', $data);
		// print_r($data['acc'][0]->jenis);
	}

	public function delete($id)
	{
		$row = $this->db->get_where('nasabah', ['nasabah_id' => $id, 'status' => 'Register']);

		if ($row->num_rows() > 0) {
			$this->db->delete('nasabah', ['nasabah_id' => $id, 'status' => 'Register']);
	...
	public function excel_by_status($status)
	{
		$this->load->helper('exportexcel');
		$namaFile = "nasabah_$status.xls";
		$judul = "nasabah";
		$tablehead = 0;
		$tablebody = 1;
		$nourut = 1;
		...
		foreach ($this->Nasabah_model->get_by_status($status) as $data) {
			$kolombody = 0;
			//ubah xlsWriteLabel menjadi xlsWriteNumber untuk kolom numeric
			xlsWriteNumber($tablebody, $kolombody++, $nourut);
			xlsWriteLabel($tablebody, $kolombody++, $data->nama_lengkap);
	...
	public function excel_Verified()
	{
		$this->load->helper('exportexcel');
		$namaFile = "nasabah_Verified.xls";
		$judul = "nasabah Verified";
		$tablehead = 0;
		$tablebody = 1;
		$nourut = 1;
		//penulisan header
		...
		foreach ($this->Nasabah_model->get_by_Verified() as $data) {
			$kolombody = 0;
	...
	// {
	// 	$data = array(
	// 		'nasabah_data' => $this->Nasabah_model->get_by_status($status),
	// 		'start' => 0
	// 	);
	// 	$this->load->view('nasabah/nasabah_print', $data);
	// }

	public function printdoc_bystatus($status)
	{
		$data = array(
			'nasabah_data' => $this->Nasabah_model->get_by_status($status),
			'start' => 0
