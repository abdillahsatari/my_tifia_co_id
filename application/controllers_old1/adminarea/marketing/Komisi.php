...
class Komisi extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model(['sales/PerhitunganKomisi_model' => 'model']);
	}
	...
	public function customrevenue()
	{
		...
		$data['page'] = 'admin_marketing/komisi/perhitungan_komisi';
	...
	public function modal_editAllowance($marketing_id)
	{
		...
		$output = '
			<form action="' . base_url() . 'adminarea/marketing/komisi/editAllowance_action/' . $marketing_id . '" method="POST" id="form">

				<p><b>Note : </b> Field yang kosong tidak akan diproses</p>

				<div class="form-group">
					<label class="" for="komisi">Komisi</label>
					<div class="input-group" id="komisi">
				...
				<div class="form-group">
					...
					<div class="input-group" id="allowance">
	...
	function fetch_list() // custom revenue
	{
		...
		foreach ($fetch_data as $r) {
			...
			$sub_array[] =  '<div class="text-center">' . ($custom_revenue['komisi'] != '' ? $custom_revenue['komisi'] . '%' : '<i>-</i>') . '</div>';
			...
			$sub_array[] =  '<div class="text-center">
								<div class="btn-group">
									<button id="modalEdit" data-href="' . base_url('adminarea/marketing/komisi/modal_editAllowance/' . $r->id) . '" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></button>
								</div>
							</div>';
			$data[] = $sub_array;
			$no++;
