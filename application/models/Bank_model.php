...
class Bank_model extends CI_Model
{
    ...
    function json()
    {
        $this->datatables->select('bank_id, nama_lengkap, nama_bank, no_rekening, jenis_rekening, kode_bank, atas_nama, currency, bank.created_date, status_bank');
        $this->datatables->from('bank');
        $this->datatables->join('nasabah', 'bank.nasabah_id=nasabah.nasabah_id');
        ...
        $this->datatables->add_column('action', anchor(site_url('adminarea/bank/read/$1'), '<i class="fa fa-search"></i>', 'class="btn btn-xs btn-primary"  data-toggle="tooltip" title="Detail"') . "  " . anchor(site_url('adminarea/bank/update/$1'), '<i class="fa fa-edit"></i>', 'class="btn btn-xs btn-warning" data-toggle="tooltip" title="Edit"'), 'bank_id');
    ...
    function get_by_id_join_bank($value)
    {
        $this->db->select('*');
        $this->db->from($this->table);
        $this->db->join('nasabah', 'nasabah.nasabah_id=' . $this->table . '.nasabah_id');
    ...
    function get_all_where($key, $value)
    {
        ...
        $this->db->where('status_bank', 'Approve');
        $this->db->order_by($this->id, $this->order);
        return $this->db->get()->result();
    }

    function get_last_where_IDR($value)
    {
        $this->db->select('*');
        $this->db->from($this->table);
    ...
    function get_last_where_USD($value)
    {
        $this->db->select('*');
        $this->db->from($this->table);
        $this->db->where('nasabah_id', $value);
