...
class Nasabah_model extends CI_Model
{
	...
	function json()
	{
		...
		$this->datatables->where('nasabah_role_id', '2')->where('is_active', '1');
		// $this->datatables->where('nasabah_role_id', '2')->where('status', 'Register');
		...
		$this->datatables->add_column(
			'action',
			//anchor(site_url('adminarea/nasabah/read/$1'),'<i class="fa fa-search"></i>', 'class="btn btn-xs btn-primary"  data-toggle="tooltip" title="Detail"')."  ".
			anchor(site_url('adminarea/nasabah/updateoff/$1'), '<i class="fa  fa-ban"></i>', 'class="btn btn-xs btn-error" data-toggle="tooltip" title="set off users"') . "  " .
				anchor(site_url('adminarea/nasabah/update/$1'), '<i class="fa fa-edit"></i>', 'class="btn btn-xs btn-warning" data-toggle="tooltip" title="Edit"') . "  " .

				anchor(site_url('adminarea/nasabah/perjanjian_nasabah/$1'), '<i class="fa fa-search"></i>', 'class="btn btn-xs btn-info" data-toggle="tooltip" title="Perjanjian Nasabah"'),
			'nasabah_id'
		);

		return $this->datatables->generate();
	}
	//  model data all users
	function jsonList()
	{
		...
		$this->datatables->add_column(
			'action',
			anchor(
				site_url('adminarea/nasabah/update/$1'),
				'<i class="fa fa-search"></i>',
				'class="btn btn-xs btn-primary"  data-toggle="tooltip" title="Detail"'
			) . "  " .
				anchor(
					site_url('adminarea/Nasabah_list_register/verifikasi_email/$1'),
					'<i class="fa fa-check-square"></i>',
					'class="btn btn-xs btn-warning" data-toggle="tooltip" title="Verifikasi Email"'
				) . "  
		   " . anchor(
					site_url('adminarea/Nasabah_list_register/verifikasi_status/$1'),
					'<i class="fa fa-check"></i>',
					'class="btn btn-xs btn-info" data-toggle="tooltip" title="verifikasi Complete"'
				),
			'nasabah_id'
		);
		return $this->datatables->generate();
	...
	function jsonRegister()
	{
		...
		$this->datatables->select('nasabah_id,nama_lengkap,gender,no_hp,email,status,status_verify, created_date');
		$this->datatables->from('nasabah');
		$this->datatables->where('nasabah_role_id', '2')->where('is_active', '1')->where('status !=', "Approved")->where('status !=', "Active");
		// ."  
		//    ".anchor(site_url('adminarea/Nasabah_list_register/verifikasi_status/$1'),
		//    '<i class="fa fa-check"></i>',
		//     'class="btn btn-xs btn-info" data-toggle="tooltip" title="verifikasi Complete"')
		$this->datatables->add_column(
			'action',
	...
	// model verifikasi
	function jsonVerifikasi()
	{
		$this->datatables->select('nasabah_id,nama_lengkap,gender,no_hp,email,status,status_verify, created_date');
		$this->datatables->from('nasabah');
		$this->datatables->where('nasabah_role_id', '2')->where('is_active', '1')->where('status !=', 'Register')->where('status !=', 'Complete');
		...
		$this->datatables->add_column(
			...
			anchor(site_url('adminarea/nasabah/read/$1'), '<i class="fa fa-search"></i>', 'class="btn btn-xs btn-primary"  data-toggle="tooltip" title="Detail"') . "  " .
				...
				anchor(
					site_url('adminarea/nasabah/review_perjanjian/$1'),
					'<i class= "fa fa-info-circle"></i>',
					'class="btn btn-xs btn-info" data-toggle="tooltip" title="Perjanjian Nasabah"'
				),
			'nasabah_id'
		);
		return $this->datatables->generate();
	...
	// model Check-in
	function jsonCheckin()
	{
		$this->datatables->select('nasabah_id,nama_lengkap,gender,no_hp,email,status,status_verify, created_date');
		$this->datatables->from('nasabah');
		$this->datatables->where('nasabah_role_id', '2')->where('is_active', '1')->where('status !=', 'Register')->where('status !=', 'Active');
		...
		$this->datatables->add_column(
			...
			anchor(site_url('adminarea/nasabah/update/$1'), '<i class="fa fa-edit"></i>', 'class="btn btn-xs btn-warning" data-toggle="tooltip" title="Edit"') . "  " .
				...
				anchor(
					site_url('adminarea/Nasabah_check_in/verifikasi_status/$1'),
					'<i class="fa fa-check-square"></i>',
					'class="btn btn-xs btn-warning" data-toggle="tooltip" title="verifikasi status"'
				) . " ",
			'nasabah_id'
		);
		return $this->datatables->generate();
	...
	// model perjanjian nasabah
	function jsonPerjanjianNasaba()
	{
		$this->datatables->select('nasabah_id,nama_lengkap,gender,no_hp,email,status,status_verify, created_date');
		$this->datatables->from('nasabah');
		$this->datatables->where('nasabah_role_id', '2')->where('is_active', '1')
			->where('status !=', 'Checking')->where('status !=', 'Register');
		$this->datatables->add_column(
			'action',
			anchor(
				site_url('adminarea/nasabah/perjanjian_nasabah/$1'),
				'<i class="fa fa-search"></i>',
				'class="btn btn-xs btn-info" data-toggle="tooltip" title="Perjanjian Nasabah"'
			),
			'nasabah_id'
	...
	// datatables set off user
	function jsonNasabaOff()
	{
		$this->datatables->select('nasabah_id,nama_lengkap,gender,no_hp,email,status,status_verify, created_date');
		$this->datatables->from('nasabah');
		$this->datatables->where('nasabah_role_id', '2')->where('is_active', '0');

		$this->datatables->add_column(
			'action',
			anchor(
				site_url('adminarea/nasabah/updateOn/$1'),
				'<i class="fa fa-check-square"></i>',
				'class="btn btn-xs btn-warning" data-toggle="tooltip" title="verifikasi status"'
			) . "  
		   " . anchor(
